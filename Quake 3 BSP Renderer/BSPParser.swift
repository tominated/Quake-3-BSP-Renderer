...
public class BSPParser {
    ...
    private func parseTextures() throws {
        bsp.textures = try readEntry(index: TEXTURES_INDEX, entryLength: 72) { parser in
    ...
    private func parsePlanes() throws {
        bsp.planes = try readEntry(index: PLANES_INDEX, entryLength: 16) { parser in
            return BSPPlane(
    ...
    private func parseNodes() throws {
        bsp.nodes = try readEntry(index: NODES_INDEX, entryLength: 36) { parser in
    ...
    private func parseLeaves() throws {
        bsp.leaves = try readEntry(index: LEAVES_INDEX, entryLength: 48) { parser in
            return BSPLeaf(
    ...
    private func parseLeafFaces() throws {
        bsp.leafFaces = try readEntry(index: LEAF_FACES_INDEX, entryLength: 4) { parser in
            return BSPLeafFace(faceIndex: parser.getNumber())
        }
    }

    private func parseLeafBrushes() throws {
        bsp.leafBrushes = try readEntry(index: LEAF_BRUSHES_INDEX, entryLength: 4) { parser in
            return BSPLeafBrush(brushIndex: parser.getNumber())
        }
    }

    private func parseModels() throws {
        bsp.models = try readEntry(index: MODELS_INDEX, entryLength: 40) { parser in
            return BSPModel(
    ...
    private func parseBrushes() throws {
        bsp.brushes = try readEntry(index: BRUSHES_INDEX, entryLength: 12) { parser in
            return BSPBrush(
    ...
    private func parseBrushSides() throws {
        bsp.brushSides = try readEntry(index: BRUSH_SIDES_INDEX, entryLength: 8) { parser in
            return BSPBrushSide(
    ...
    private func parseVertices() throws {
        bsp.vertices = try readEntry(index: VERTICES_INDEX, entryLength: 44) { parser in
            return BSPVertex(
    ...
    private func parseMeshVerts() throws {
        bsp.meshVerts = try readEntry(index: MESH_VERTS_INDEX, entryLength: 4) { parser in
            return BSPMeshVert(vertexIndexOffset: parser.getNumber())
        }
    }

    private func parseEffects() throws {
        bsp.effects = try readEntry(index: EFFECTS_INDEX, entryLength: 72) { parser in
            guard let name = parser.getString(maxLength: 64) else {
    ...
    private func parseFaces() throws {
        bsp.faces = try readEntry(index: FACES_INDEX, entryLength: 104) { parser in
    ...
    private func parseLightVols() throws {
        bsp.lightVols = try readEntry(index: LIGHT_VOLS_INDEX, entryLength: 8) { parser in
            return BSPLightVol(
